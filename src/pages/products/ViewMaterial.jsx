...
export default function ViewMaterial({onBack}) {
...
console.log(data)
  const totalPages = Math.ceil(data.length / materialsPerPage);
  ...
  const handleSaveEdit = (index) => {
    ...
    const newData = [...data];
    newData[(currentPage - 1) * materialsPerPage + index].material = editValue;
    setData(newData);
  ...
  const handleDelete = (index) => {
    const globalIndex = (currentPage - 1) * materialsPerPage + index;
...
Products              </span>
      ...
      <div className="w-full">
        ...
        <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {paginatedCategories.map((mat, index) => {
